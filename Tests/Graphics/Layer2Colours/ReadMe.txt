Draw all three basic layers (Sprite, Layer2, ULA) in all combinations affecting priority
and transparency.
...
So the TEST-area for particular layer ordering is always on the right end of screen in
6x4 characters area (one character = 8x8 pix). This output is composed from all the
layers, and it's result shows the emulator/board result.
...
The 1x1 chequered black/white areas in sprites depict no-sprite-pixel involved situation.
...
amount of combinations in the test, these are possibilities are ignored)
...
; "xxyyzz" three states contributing to final pixel, "ww" after is visible result, where
